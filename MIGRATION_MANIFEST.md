...
Since the migration has matured, the C++ implementation is the canonical wire contract: `main.py` served as the architectural reference that seeded this codebase and is retained as an immutable historical record, but it is no longer treated as an interop requirement. New protocol work proceeds from the C++ contract.
...
- **Independent nonce replay domains** — `PQVPNNode.check_and_record_nonce`, `PQVPNNode.handle_relay` local delivery (reference behavior at `main.py:3187-3229`). The reference keeps one shared per-session counter window and accepts counters up to 1024 below the high-water mark when not yet seen. The C++ port tightens each domain to strict monotonicity (`tests/test_node_check_and_record_nonce.cpp`, "Out-of-order within window rejected") and splits replay state into two independent domains: direct tunnel frames (TUNNEL_DATA + PING/PONG, AAD-bound to the 16-byte outer header) and onion RELAY layers peeled by this node (AAD-bound to the five-part relay AAD). Both frame types draw from the one monotonic send counter, so nonces stay unique per session key; the split stops a freshly peeled layer's higher counter from evicting the lower counter of the tunnel data frame it carries on the same source-destination session — which would otherwise make onion delivery fail deterministically at the final hop. Cross-domain replay is impossible because the two domains are AEAD-bound to structurally different AADs. Evidence: `tests/test_handle_relay.cpp` (`LocalDeliveryDeliversDecryptedPacket` builds both frames on one shared source-destination session; `MultiHopDeliversInnerDataOnSharedDestinationSession` drives a real first→second UDP forward and observes delivery of the inner frame), `tests/test_node_check_and_record_nonce.cpp` (cross-domain independence plus per-domain strict monotonicity).
- **Nonce counter starts at 1** — `PQVPNNode::check_and_record_nonce`, all nonce-generating send sites (`main.py:3290-3351` builder; `main.py:3187-3229` peeler). The reference's first AEAD nonce in a session uses counter 0 (the builder serializes `nonce_send` before incrementing from an initial value of 0), and its peeler accepts it through the out-of-order tolerance branch (`nonce_recv=0`, `highest - counter <= replay_window_size`). The C++ port starts every domain's send counter at 1 (pre-increment) and rejects any counter at or below the domain's high-water mark, whose initial value is 0 — so there is no "first nonce" special case to tolerate. This keeps strict monotonicity uniform from a session's very first frame: main.py needs its tolerance branch precisely because its builder can emit 0. Consequence (accepted): a Python-reference sender's first onion layer in a fresh session is rejected by a C++ node; C++-to-C++ and C++-to-Python traffic are unaffected, since counters ≥1 are accepted on both sides. Per the section note above, the C++ convention is canonical. Evidence: `tests/test_node_check_and_record_nonce.cpp` (test case 11 pins counter-zero rejection on a fresh domain and first acceptance at exactly counter 1; test case 4 accepts counter 1 as the first nonce), `tests/test_handle_relay.cpp` (end-to-end relay delivery uses counters ≥1 throughout).
- **Route-exclusion gate on peer admission** — `PQVPNNode.register_peer_from_hello`, `PQVPNNode::establish_hybrid_session` (no reference counterpart; full-tunnel route exclusions are a post-migration addition). When the platform hook reports that a new peer's /32 exclusion could not be installed while the TAP default route is active, admitting that peer would send its UDP transport into the adapter and loop it through re-encryption. Registration therefore fails closed (HELLO rejected — retryable once routing recovers) and hybrid session establishment returns no session, instead of recording a peer without a working exclusion. Removals remain best-effort so teardown is never blocked by cleanup failures. Evidence: `tests/test_peer_route_manager.cpp` ("HELLO registration is rejected when its route exclusion fails", "session establishment is refused when its route exclusion fail").
...
- **Unilateral local rekey refused** — `PQVPNNode.maintenance_tick`/`session_maintenance` (reference behavior at `main.py:4610-4725`). The reference maintenance installs freshly derived traffic keys when the rotation threshold is reached, but its `perform_rekey` derives them from locally generated random entropy (`os.urandom`) that the peer can never know. Swapping such keys in unilaterally destroys an authenticated channel (every later frame fails AEAD at the peer) with no security gain. The C++ maintenance therefore keeps the working keys intact and performs no unilateral swap until an authenticated key exchange exists; `RekeyManager` remains available as a tested primitive for that future exchange. Evidence: `tests/test_peer_liveness.cpp` ("maintenance never swaps keys unilaterally when the rekey threshold is reached") and `tests/test_key_rotation_init.cpp` (the rotation primitives themselves are unchanged).
