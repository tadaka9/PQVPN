...
- **Relay sender binding** — `PQVPNNode.handle_relay` (`main.py:3392-3479`). The reference handler takes no sender address, so a relay layer is accepted from any source once its session material verifies. The C++ handler requires the datagram's sender to equal the resolved session's registered peer endpoint — the same posture as the direct tunnel path in `datagram_received` — because every deliverable relay layer arrives directly from the onion source, while stolen session material submitted from an unregistered address is blocked. Multi-peel chains (a second relay peeling a forwarded layer) are not a supported topology in either implementation: main.py's forward branch re-sends the peeled content as raw `session_hint+nonce+ciphertext` with no outer frame (`main.py:3456-3479`), yet every receiver requires a 16-byte outer header to dispatch at all (`main.py:4493-4508`), so a forwarded layer is dropped before `handle_relay` runs. Sender binding therefore regresses no deliverable path; it only tightens the origin check on the layers that do arrive. Evidence: `tests/test_handle_relay.cpp` (RelayLayerFromForeignEndpointIsRejected pins the rejection; ForwardedOnionLayerIsUndeliverableAtTheDispatcher shows a first peeler accepts from its source and forwards, while the second node cannot deliver the headerless layer through its real dispatcher; all acceptance tests deliver from the registered endpoint).
...
Note on relay forwarding and sender binding: in both implementations a non-final relay re-sends the peeled content as raw `session_hint+nonce+ciphertext` with no 16-byte outer frame (`main.py:3456-3479`; C++ `handle_relay` forward branch), while every dispatcher requires that header to route anything (`main.py:4493-4508`; C++ `datagram_received`). A forwarded onion layer is therefore undeliverable in the reference itself, so no valid topology ever delivers a relay layer from an intermediate hop. Sender binding applies only to layers received directly from their source — the first peeler's session (source ↔ first peeler) — and blocks stolen material injected from unregistered addresses without weakening any deliverable path.
