...
- **Relay sender binding** — `PQVPNNode.handle_relay` (`main.py:3392-3479`). The reference handler takes no sender address, so a relay layer is accepted from any source once its session material verifies. The C++ handler binds the datagram's origin to the resolved session in two tiers: hop one must come directly from the endpoint the session was established with — the same posture as the direct tunnel path in `datagram_received`; forwarded layers (hop two and later) necessarily arrive from the preceding relay, so they are accepted when the sender matches a mesh-registered peer address (the forwarder resolved this node's address through its own mesh to send here, and under symmetric discovery that relay is registered here too). In both tiers the layer remains bound to its creator by the source-peeler AEAD key, and the monotonic nonce window rejects replays, so an unregistered address can neither forge nor replay a valid layer. Evidence: `tests/test_handle_relay.cpp` (`HandleRelay.RelayLayerFromForeignEndpointIsRejected` pins hop-one rejection; `HandleRelay.ForwardedLayerFromUnregisteredSenderIsRejected` shows the binding still rejects forwarded layers from unknown senders; `HandleRelay.MultiPeelDeliversThroughRealDispatchers` shows a discovery-registered relay origin is accepted and peeled).
