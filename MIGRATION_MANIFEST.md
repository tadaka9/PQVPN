...
Note on relay forwarding and sender binding: forwarded content always re-origins from the intermediate hop, so sender binding applies to the layer's own session (source ↔ first peeler), which in every topology produced by either implementation's builders receives directly from the source.
