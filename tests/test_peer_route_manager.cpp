...
TEST_CASE("pruning keeps the exclusion while a mesh entry still uses the same endpoint", "[routing][peerroutes][node]") {
    pqvpn::PQVPNNode node("test_config.toml");
    std::vector<std::pair<asio::ip::udp::endpoint, bool>> notifications;
    node.set_peer_route_hook([&notifications](const asio::ip::udp::endpoint& address, const bool add) {
        notifications.emplace_back(address, add);
    ...
    // Stale session at 203.0.113.20:443 — past SESSION_TIMEOUT, so it is pruned.
    auto stale = std::make_shared<pqvpn::PQVPNNode::Session>();
    stale->session_id = {0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC, 0xDE, 0xF0};
    stale->remote_addr = peer_endpoint("203.0.113.20", 443);
    stale->state = pqvpn::PQVPNNode::SessionState::ESTABLISHED;
    ...
    // A live relay-capable mesh entry on the SAME endpoint (203.113.20:443). Relay
    // forwarding can still send to it, so its exclusion must survive the session prune.
    pqvpn::PQVPNNode::PeerInfo info;
    info.peer_id = {0x11, 0x22};
    info.address = peer_endpoint("203.0.113.20", 443);
    ...
    REQUIRE(node.sessions_by_peer_id.empty());
    // ...but the mesh entry that can still send to this exact endpoint survives, and its
    // exclusion must NOT have been released (no add=false notification).
    REQUIRE(node.mesh.peers.size() == 1);
    REQUIRE(notifications.empty()); // a consumer still uses this endpoint
}

TEST_CASE("pruning releases the session's endpoint when only another port on the address remains", "[routing][peerroutes][node]") {
    pqvpn::PQVPNNode node("test_config.toml");
    std::vector<std::pair<asio::ip::udp::endpoint, bool>> notifications;
    node.set_peer_route_hook([&notifications](const asio::ip::udp::endpoint& address, const bool add) {
        notifications.emplace_back(address, add);
    ...
    // Stale session at 203.113.20:443 — past SESSION_TIMEOUT, so it is pruned.
    auto stale = std::make_shared<pqvpn::PQVPNNode::Session>();
    stale->session_id = {0x12, 0x34, 0x56, 0x78, 0x9A, 0xBC, 0xDE, 0xF0};
    stale->remote_addr = peer_endpoint("203.0.113.20", 443);
    stale->state = pqvpn::PQVPNNode::SessionState::ESTABLISHED;
    ...
    // A live mesh entry on the SAME address but a DIFFERENT port (51820). It is an
    // independent endpoint reference in PeerRouteManager and keeps the shared /32 OS
    // route alive — but it does NOT keep this session's :443 reference, which must be
    // released so no stale endpoint lingers after its consumer is gone.
    pqvpn::PQVPNNode::PeerInfo info;
    info.peer_id = {0x11, 0x22};
    ...
    const bool completed_cleanly = !failure; // exception_ptr converts to bool
    REQUIRE(completed_cleanly);              // maintenance ran without throwing
    // The stale session is pruned and its :443 endpoint reference released...
    REQUIRE(node.sessions_by_peer_id.empty());
    REQUIRE(notifications.size() == 1);
    REQUIRE(notifications.front().first == peer_endpoint("203.0.113.20", 443));
    REQUIRE_FALSE(notifications.front().second); // add=false: the :443 reference is dropped
    // ...while the mesh entry (a different endpoint) survives to hold the shared /32.
    REQUIRE(node.mesh.peers.size() == 1);
}
