...
class ZeroKnowledgeAuth:
    ...
    def issue_credential(self, peer_id: bytes) -> bytes:
        """Issue authentication credential (serialized via TokenRefreshManager)."""
        from pqvpn.token_lock import get_default_manager
        mgr = get_default_manager()

        def _issue():
            # Double-check existing credential to avoid unnecessary re-issuance
            if peer_id in self.credential_store:
                return self.credential_store[peer_id]
            credential = hashlib.sha256(peer_id + os.urandom(32)).digest()
            self.credential_store[peer_id] = credential
            return credential

        # Use sync lock because this code path may be called from sync contexts
        return mgr.refresh_token_sync(peer_id, _issue)
