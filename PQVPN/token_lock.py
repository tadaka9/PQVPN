...
Usage (async):
  from token_lock import TokenRefreshManager
  mgr = TokenRefreshManager()
  async def do_refresh():
      # actual refresh logic that returns the new token
      return await some_refresh_coroutine()
  token = await mgr.refresh_token(peer_id, do_refresh)

Usage (sync fallback):
  token = mgr.refresh_token_sync(peer_id, lambda: blocking_refresh())

This module is intentionally minimal and conservative — it does not change
existing token storage semantics. It only exposes a locking primitive and
convenience helpers to be wired into credential/token refresh paths.
"""
