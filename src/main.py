...
# Prefer implementations from pqvpn.crypto if available (non-destructive).
# This allows the crypto helpers to live in a separate module while keeping
...
try:
    ...
    for _name in (
        ...
        if hasattr(_pqcrypto, _name):
            globals()[_name] = getattr(_pqcrypto, _name)
except Exception:
    # Best-effort: if the crypto module fails to import or initialize,
    # keep the in-file implementations intact.
    pass
